...
# ======================================================================
# NB3 V3 — Semantic separation via k-fold CV (robust estimate on small data)
# ======================================================================
nb3v3 = [
 md("""# PICKO Research · NB3 V3 — **Semantic separation, k-fold CV**: a robust estimate on little data"""),
 BOOTSTRAP_MD, BOOTSTRAP, SETUP_MD, SETUP,
 md("""## 2 · Why k-fold here

NB3 V2 answered *can the model learn to route by domain meaning?* on a **single** 20-example test split —
too few points to trust. **V3 runs the same experiment as k-fold cross-validation:** every source-free
query serves as test **exactly once**, and we report the mean over folds with a std error bar. This turns
one shaky number into a distribution, which is what small data needs.

Per fold we **check there is no leakage** (no test query appears in the fold's train set), train a fresh
2-tool model on the training folds, and evaluate it on the held-out fold. `before` = the existing
name-trained model on the same held-out fold (no semantic training); `after` = the fold's freshly trained
model. We aggregate all folds at the end."""),
 nb3v2[6], nb3v2[7],   # reuse V2's "build the source-free dataset" markdown + code (identical)
 md("## 4 · Configure"),
 co('''NB_DIR = os.path.join(OUT_DIR, "nb3v3"); os.makedirs(NB_DIR, exist_ok=True)   # this notebook's outputs
...
for i, f in enumerate(FOLDS):
 ...
 md("""## 6 · Run the folds — train, **check no leakage**, evaluate on the held-out fold

For each fold: build train (the other folds) + test (this fold); **assert no test query is in train**;
train a fresh 2-tool model on train; predict on the held-out test; also run the `before` baseline on it.
Resumable — a finished fold is loaded from `nb3v3/kfold_results.json` and its training is skipped."""),
 co('''import contextlib, io
...
for i in range(K):
    ...
    try:
        ...
        # --- leakage guard: no held-out query may appear in the training folds ---
        trq = {e["query"] for e in train}
        overlap = [e["query"] for e in test if e["query"] in trq]
        assert not overlap, f"LEAKAGE fold {i}: {len(overlap)} test queries also in train"
        log(f"fold {i}: train={len(train)} test={len(test)} · leakage check OK (0 overlap)")
...
Because every source-free query is tested exactly once by a model that never trained on it (leakage checked
per fold), the k-fold mean is a far more trustworthy estimate than the single-split V2. If **after** sits
clearly above **before** and above the 0.5 chance line across folds — with a small std — the domain
distinction is genuinely learnable; a large std or overlap with chance means the signal is weak for a 26M
model on this little data."""),
]
...
if __name__ == "__main__":
    ...
    write(nb3v2, "nb3v2_semantic_separation.ipynb")
    write(nb3v3, "nb3v3_kfold_semantic.ipynb")
